...
public class CalendarDataSO : ScriptableObject
{
    ...
    {
        if (currentDayIndex < seasonsList[currentSeasonIndex].monthList[currentMonthIndex].weeksList[currentWeekIndex].dayList.Count)
        {
            currentDayIndex++;
            onDayChange?.Invoke(currentDayIndex);
        }
        else if (currentDayIndex == seasonsList[currentSeasonIndex].monthList[currentMonthIndex].weeksList[currentWeekIndex].dayList.Count - 1)
        {
            currentDayIndex = 0;
            OnNextWeek();
            onDayChange?.Invoke(currentDayIndex);
    ...
    {
        if (currentWeekIndex < seasonsList[currentSeasonIndex].monthList[currentMonthIndex].weeksList.Count -1)
        {
            currentWeekIndex++;
            onWeekChange?.Invoke(currentWeekIndex);
        }
        else if (currentWeekIndex == seasonsList[currentSeasonIndex].monthList[currentMonthIndex].weeksList.Count - 1)
        {
            currentWeekIndex = 0;
            OnNextMonth();
            onWeekChange?.Invoke(currentWeekIndex);
    ...
    {
        if (currentMonthIndex < seasonsList[currentSeasonIndex].monthList.Count - 1)
        {
            currentMonthIndex++;
            onMonthChange?.Invoke(currentMonthIndex);
        }
        else if (currentMonthIndex == seasonsList[currentSeasonIndex].monthList.Count - 1)
        {
            currentMonthIndex = 0;
            OnNextSeason();
            onMonthChange?.Invoke(currentMonthIndex);
    ...
    {
        return seasonsList[currentSeasonIndex].monthList[monthIndex].monthName;
    }
}